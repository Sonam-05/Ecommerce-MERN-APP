...
const CreateTag = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.post(`${process.env.REACT_APP_API}/api/v1/tag/create-tag`, { name, slug }, {
                headers: {
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.put(`${process.env.REACT_APP_API}/api/v1/tag/update-tag/${selected._id}`, { name: updatedName, slug : updatedSlug }, {
        headers: {
